  ...
  (let ((target (if url-format
                    ...
                    (ecase (dm:collection object)
                      (note (if (string= "true" (post/get "in-detail"))
                                (note-url object)
                                (track-note-url object)))
                      (entry (if (string= "true" (post/get "in-detail"))
                                 (entry-url object)
                                 (track-entry-url object)))
                      (project (project-url object))
                      (track (track-url object))
                      (trace (trace-url object))
                      (snapshot (snapshot-url object))
                      (attachment (entry-url (dm:field object "entry")))))))
