...
(defun submit (project user-id &key description attachments key secret token token-secret parameters (api-base "https://feedback.tymoon.eu/api/"))
  (let ((parameters (append `(("project" . ,project)
                              ("user-id" . ,(string user-id))
                              ("description" . ,(or description "")))
                            (gather-system-info)
                            parameters))
        (client (make-instance 'client :key key :secret secret :token token :token-secret token-secret :api-base api-base)))
    (post-file client "feedback/entry/new" attachments)))
